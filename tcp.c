...
tcp_dump(const uint8_t *data, size_t len)
{
  ...
  fprintf(stderr, CYAN "TCP header dump\n" WHITE);
  fprintf(stderr, "   " MAZENTA "src" WHITE ": %u\n", ntoh16(hdr->src));
  fprintf(stderr, "   " MAZENTA "dst" WHITE ": %u\n", ntoh16(hdr->dst));
  fprintf(stderr, "   " MAZENTA "seq" WHITE ": %u\n", ntoh32(hdr->seq));
  fprintf(stderr, "   " MAZENTA "ack" WHITE ": %u\n", ntoh32(hdr->ack));
  fprintf(stderr, "   " MAZENTA "off" WHITE ": 0x%02x (%d)\n", hdr->off, (hdr->off >> 4) << 2);
  fprintf(stderr, "   " MAZENTA "flg" WHITE ": 0x%02x (" YELLOW "%s" WHITE ")\n", hdr->flg, tcp_flg_ntoa(hdr->flg));
  fprintf(stderr, "   " MAZENTA "wnd" WHITE ": %u\n", ntoh16(hdr->wnd));
  fprintf(stderr, "   " MAZENTA "sum" WHITE ": 0x%04x\n", ntoh16(hdr->sum));
  fprintf(stderr, "    " MAZENTA "up" WHITE ": %u\n", ntoh16(hdr->up));
#ifdef HEXDUMP
...
tcp_retransmit_queue_cleanup(struct tcp_pcb *pcb)
{
  ...
  while(1) {
    ...
    debugf(YELLOW "remove" WHITE ", seq=%u, flags=%s, len=%u", entry->seq, tcp_flg_ntoa(entry->flg), entry->len);
    memory_free(entry);
  }
  return;
}
...
tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags, uint8_t *data, size_t len, struct ip_endpoint *local, struct ip_endpoint *foreign)
{
  ...
  case TCP_PCB_STATE_LISTEN:
    ...
    if(TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
      ...
      /* (combined with SYN) will be procesed in the SYN-RECEIVED state. */
  ...
  case TCP_PCB_STATE_SYN_SENT:
    ...
    if(TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
      ...
      if(pcb->snd.una > pcb->iss) {
        ...
        pcb->state = TCP_PCB_STATE_SYN_RECEIVED;
        tcp_output(pcb, TCP_FLG_SYN | TCP_FLG_ACK, NULL, 0);
        /* ignore: if there are other controls or text in the segment, queue them for processing after the ESTABLISHED state has beed reached */
        return;
      }
    }
...
tcp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
  ...
  if(src == iface->broadcast || dst == iface->broadcast) {
    errorf("cannot use broadcast address, src=%s, dst=%s",
  ...
  for(pcb = pcbs; pcb < tailof(pcbs); pcb++) {
    ...
    if(pcb->state == TCP_PCB_STATE_TIME_WAIT) {
      if(timercmp(&now, &pcb->tw_timer, >) != 0) {
        debugf("timewait has elapsed: local=" RED "%s" WHITE ":" YELLOW "%d" WHITE ", foreign=" RED "%s" WHITE ":" YELLOW "%d" WHITE,
...
tcp_open_rfc793(struct ip_endpoint *local, struct ip_endpoint *foreign, int active)
{
  ...
  if(active) {
    debugf("active open: local=" RED "%s" WHITE ":" YELLOW "%d" WHITE ", foreign=" RED "%s" WHITE ":" YELLOW "%d" WHITE ", connecting...",
    ...
    debugf("passive open: local=" RED "%s" WHITE ":" YELLOW "%d" WHITE ", waiting for connection...",
        ip_addr_ntop(local->addr, addr1, sizeof(addr1)), ntoh16(local->port));
    pcb->local = *local;
    if(foreign) {
      pcb->foreign = *foreign;
...
AGAIN:
  ...
  id = tcp_pcb_id(pcb);
  debugf("connection established: local=" RED "%s" WHITE ":" YELLOW "%d" WHITE ", foreign=" RED "%s" WHITE ":" YELLOW "%d" WHITE,
      ip_addr_ntop(pcb->local.addr, addr1, sizeof(addr1)), ntoh16(pcb->local.port),
      ip_addr_ntop(pcb->foreign.addr, addr2, sizeof(addr2)), ntoh16(pcb->foreign.port));
  mutex_unlock(&mutex);
  return id;
...
tcp_connect(int id, struct ip_endpoint *foreign)
{
  ...
  if(local.addr == IP_ADDR_ANY) {
    ...
    debugf("select source address: " RED "%s" WHITE, ip_addr_ntop(iface->unicast, addr, sizeof(addr)));
  ...
  if(!local.port) {
    for(p = TCP_SOURCE_PORT_MIN; p <= TCP_SOURCE_PORT_MAX; p++) {
      ...
      if(!tcp_pcb_select(&local, foreign)) {
        debugf("dynamic assign source port: " YELLOW "%d", ntoh16(local.port));
...
tcp_bind(int id, struct ip_endpoint *local)
{
  ...
  pcb->local = *local;
  debugf("success: local=" RED "%s" WHITE ":" YELLOW "%d" WHITE, ip_addr_ntop(pcb->local.addr, ep, sizeof(ep)), ntoh16(pcb->local.port));
  mutex_unlock(&mutex);
  return 0;
}
...
tcp_close(int id)
{
  ...
  case TCP_PCB_STATE_CLOSE_WAIT:
    tcp_output(pcb, TCP_FLG_ACK | TCP_FLG_FIN, NULL, 0);
    pcb->snd.nxt++;
    pcb->state = TCP_PCB_STATE_LAST_ACK; /* RFC793 says "enter CLOSING state", but it seems to be LAST_ACK state */
    break;
  case TCP_PCB_STATE_CLOSING:
  case TCP_PCB_STATE_LAST_ACK:
  case TCP_PCB_STATE_TIME_WAIT:
