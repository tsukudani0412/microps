...
ether_tap_addr(struct net_device *dev) 
{
  ...
  strncpy(ifr.ifr_name, PRIV(dev)->name, sizeof(ifr.ifr_name));
...
ether_tap_open(struct net_device *dev)
{
  ...
  /* Set Asynchronous I/O signal destination */
...
ether_tap_init(const char *name, const char *addr)
{
  ...
  infof("ethernet device initialized, dev=" GREEN "%s" WHITE, dev->name);
  return dev;
}
