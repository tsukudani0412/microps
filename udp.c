...
udp_dump(const uint8_t *data, size_t len) 
{
  ...
  fprintf(stderr, CYAN "UDP header dump\n" WHITE);
  fprintf(stderr, "   " MAZENTA "src" WHITE ": %u\n", ntoh16(hdr->src));
  fprintf(stderr, "   " MAZENTA "dst" WHITE ": %u\n", ntoh16(hdr->dst));
  fprintf(stderr, "   " MAZENTA "len" WHITE ": %u\n", ntoh16(hdr->len));
...
int udp_bind(int id, struct ip_endpoint *local)
{
  ...
  if(exist) {
    mutex_unlock(&mutex);
    errorf("address and port in in use: %s", ip_endpoint_ntop(local, ep1, sizeof(ep1)));
    return -1;
  }
  ...
  if(!pcb->local.port) { // source port auto select
    for(p = UDP_SOURCE_PORT_MIN; p <= UDP_SOURCE_PORT_MAX; p++) {
      if(!udp_pcb_select(local.addr, hton16(p))) {
        ...
        debugf("dynamic assign local port, port=" YELLOW "%d" WHITE, p);
        break;
      }
    }
    if(!pcb->local.port) {
      debugf("failed to dynamic assign local port, addr=" RED "%s" WHITE,
