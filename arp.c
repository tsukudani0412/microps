...
arp_cache_delete(struct arp_cache *cache) 
{
  ...
  debugf(YELLOW "DELETE" WHITE ": pa=" RED "%s" WHITE ", ha=" RED "%s" WHITE,
      ip_addr_ntop(cache->pa, addr1, sizeof(addr1)), ether_addr_ntop(cache->ha, addr2, sizeof(addr2)));
...
arp_cache_update(ip_addr_t pa, const uint8_t *ha)
{
  ...
  debugf(YELLOW "UPDATE" WHITE ": pa=" RED "%s" WHITE ", ha=" RED "%s",
...
arp_cache_insert(ip_addr_t pa, const uint8_t *ha)
{
  ...
  memcpy(cache->ha, ha, ETHER_ADDR_LEN);
  gettimeofday(&cache->timestamp, NULL);
  debugf(YELLOW "INSERT" WHITE ": pa=" RED "%s" WHITE ", ha=" RED "%s",
      ip_addr_ntop(pa, addr1, sizeof(addr1)), ether_addr_ntop(ha, addr2, sizeof(addr2)));
  return cache;
}
...
arp_resolve(struct net_iface *iface, ip_addr_t pa, uint8_t *ha)
{
  ...
  cache = arp_cache_select(pa);
  if(!cache) {
    debugf("cache not found, pa=" RED "%s" WHITE, ip_addr_ntop(pa, addr1, sizeof(addr1)));
  ...
  mutex_unlock(&mutex);
  debugf("resolved, pa=" RED "%s" WHITE ", ha=" RED "%s" WHITE,
      ip_addr_ntop(pa, addr1, sizeof(addr1)), ether_addr_ntop(ha, addr2, sizeof(addr2)));
